...
const RegisterChat = () => {
    ...
    useEffect(() => {
        ...
        fetchChatHistory();
        const newSocket  = io('http://localhost:8000', { path: '/socket.io', transports: ['websocket'] });
        newSocket.on('msg', (data) => {
            if (data.senderId !== currentUserId) { setMessages((prevMessages) => [...prevMessages, data]); }
        });
        setSocket(newSocket);
    ...
    const handleBack = () => { nav('/Register_reco'); };
